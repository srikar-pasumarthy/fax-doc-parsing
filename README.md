...
- `app.py` — FastAPI app exposing `/api/documents`, `/api/documents/{id}`,
  `/api/documents/{id}/pages/{p}/image.png`, and the static UI.
...
The app's service principal needs `USE CATALOG`/`USE SCHEMA`, `SELECT` on the
table, and `READ VOLUME` on the volume.
